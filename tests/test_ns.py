...
def test_ns_contour_valid():
    ...
    for t, d, v, c, i, b in zip(times, durations, values, confidences, ids, voicing):
        ann.append(
            time=t, duration=d, value={"pitch": v, "id": i, "voiced": b}, confidence=c
        )
...
@parametrize(
    "value",
    ["{}:maj".format(_) for _ in [42, "H", "a", "F1", True, None]]
    + ["C:{}".format(_) for _ in [64, "z", "mj", "Ab", "iiii", False, None]]
    + ["C/{}".format(_) for _ in ["A", 7.5, "8b"]]
    + [None],
)
...
@parametrize(
    "value",
    ["{}:maj".format(_) for _ in [42, "X", "a", "F1", True, None]]
    + ["C:{}".format(_) for _ in [64, "z", "mj", "Ab", "iiii", False, None]]
    + ["C/{}".format(_) for _ in ["A", 7.5, "8b"]]
    + [None],
)
...
@parametrize(
    "key", ["midi_pitch", "morph_pitch", "staff", "pattern_id", "occurrence_id"]
)
