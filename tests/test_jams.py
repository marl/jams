...
@pytest.mark.parametrize(
    "curator", [None, jams.Curator(name="nobody", email="none@none.com")]
)
...
def test_annotation(namespace, tag_data, ann_metadata, ann_sandbox):
    ann = jams.Annotation(
        namespace, data=tag_data, annotation_metadata=ann_metadata, sandbox=ann_sandbox
    )
...
def test_filemetadata():
    ...
    meta = dict(
        title="Test track", artist="Test artist", release="Test release", duration=31.3
    )
...
def test_filemetadata_validation_warning():
    ...
    fm = jams.FileMetadata(
        title="Test track", artist="Test artist", release="Test release", duration=None
    )
...
def test_jams(tag_data, file_metadata, ann_sandbox):
    ...
    jam = jams.JAMS(
        annotations=annotations, file_metadata=file_metadata, sandbox=ann_sandbox
    )
...
@pytest.mark.parametrize(
    "on_conflict,exception",
    [("fail", jams.JamsError), ("bad_fail_mdoe", jams.ParameterError)],
)
...
def test_jams_bad_annotation_warnings():
    ...
    with pytest.warns(
        UserWarning, match=".*(is not a well-formed JAMS Annotation).*"
    ) as out:
    ...
    for ext in ["jams", "jamz"]:
        for validate in [False, True]:
            for strict in [False, True]:
                jams.load("{:s}.{:s}".format(fn, ext), validate=validate, strict=strict)
...
def test_annotation_trim_no_duration():
    ...
    expected_ann = jams.Annotation(
        namespace, data=expected_data, time=5.0, duration=3.0
    )
...
def test_annotation_trim_complete_overlap():
    ...
    assert ann_trim.sandbox.trim == [
        {"start_time": 8, "end_time": 12, "trim_start": 8, "trim_end": 12}
    ]
    ...
    expected_ann = jams.Annotation(
        namespace, data=expected_data, time=8.0, duration=4.0
    )
...
def test_annotation_trim_partial_overlap_beginning():
    ...
    assert ann_trim.sandbox.trim == [
        {"start_time": 1, "end_time": 8, "trim_start": 5, "trim_end": 8}
    ]
    assert ann_trim.namespace == ann.namespace
    assert ann_trim.annotation_metadata == ann.annotation_metadata
...
def test_annotation_trim_partial_overlap_end():
    ...
    assert ann_trim.sandbox.trim == [
        {"start_time": 8, "end_time": 20, "trim_start": 8, "trim_end": 15}
    ]
    assert ann_trim.namespace == ann.namespace
    assert ann_trim.annotation_metadata == ann.annotation_metadata
    ...
    expected_ann = jams.Annotation(
        namespace, data=expected_data, time=8.0, duration=7.0
    )
...
def test_annotation_trim_multiple():
    ...
    expected_ann = jams.Annotation(
        namespace, data=expected_data, time=8.0, duration=2.0
    )
...
def test_annotation_slice():
    ...
    expected_ann = jams.Annotation(
        namespace, data=expected_data, time=2.0, duration=5.0
    )
    assert ann_slice.time == expected_ann.time
    assert ann_slice.duration == expected_ann.duration
